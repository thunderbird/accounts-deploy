...
> This runbook applies to the legacy ECS/Pulumi deployment in
> `thunderbird/thunderbird-accounts`, not the EKS deployment managed by this
> repository.
...
environment files in documentation, commits, PRs, screenshots, or shared logs.
...
- Pulumi CLI authenticated to Pulumi Cloud with access to `thunderbird/prod`
  and its stack secrets.
...
1. Read the [release notes](https://github.com/keycloak/keycloak/releases) and
   [migration guide](https://www.keycloak.org/docs/latest/upgrading/#migration-changes)
   for every version crossed. Check database, OIDC, authentication-flow, theme,
   startup-option, and server-SPI changes.
2. Update both image stages in `Dockerfile.keycloak`:
...
3. If an API used by `keycloak-mfa-rest` changed, release a compatible provider
   and update its version and checksum. Update config-cli only when required by
   its compatibility notes or tests.
4. Validate:
...
5. Run the end-to-end suite from `test/e2e/README.md`. Ensure the theme, OIDC,
   introspection, MFA/recovery, and mail-authentication tests did not skip.
6. Open and merge the upgrade PR with upstream links and test results.
...
Record the current `.keycloak_image` in `pulumi/config.prod.yaml` for rollback.
Create and merge a small PR changing only that anchor to the artifact's exact
`ecr_tag`. Run production Pulumi from a clean checkout of merged `main`.
