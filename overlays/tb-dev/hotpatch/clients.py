...
class MailClient:
    ...
    def get_telemetry(self):
        ...
        v0.16 HOTPATCH (tb-dev): Stalwart v0.16 removed the REST management API,
        so PATCH /api/telemetry/metrics is gone (404). Use GET /api/account -- a
        surviving endpoint that returns the current token's permission set -- as a
        lightweight auth + reachability probe. 200 => healthy; 401/5xx => raise.
        No {data}/{error} envelope here, so skip _raise_for_error.
        """
        response = requests.get(
